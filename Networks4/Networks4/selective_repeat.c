...
{
  if(strcmp(nodeinfo.nodename,"Sender")==0 && flag_sen==1 && total_msg<10000) {
        while(flag_sen==1)
      {
  if(count==N){
    count=0;
  ...
  else if(strcmp(nodeinfo.nodename,"Receiver")==0 && flag_sen_recv==1) {
    packet_ack *ack;
    unsigned int length;
...
static void frame_arrived(CnetEvent ev, CnetTimerID timer, CnetData data)
{
  if(strcmp(nodeinfo.nodename,"Sender")==0) {
    ...
    packet_data *frame;
    unsigned int length=sizeof(packet_data);
    int link,i;
    frame=(packet_data *)malloc(1*sizeof(packet_data));
